...
@section('content')
    ...
    <table id="transmitalSlip_reported_unreported">
            ...
            <tfoot>
                <tr>
                    <td class="font-bold">UNREPORTED</td>
                </tr>
            </tfoot>
    </table>
   ...
   <div class="overflow-x-auto mt-1">
        <table class="min-w-full divide-y divide-gray-200" id="reported_unreportedTable">
                ...
                @foreach ($unreports as $report)
                    ...
                    <tr> 
                        {{--END edit and delete unreports END--}}
                ...
                @php
                    $grandTotal_Gross_Amt = $unreports->sum('gross_amt');
                    $grandTotal_Net_Of_Vat = $unreports->sum('net_of_vat');
                    $grandTotal_Input_Vat = $unreports->sum('input_vat');
                    $grandTotal_Non_Vat = $unreports->sum('non_vat');
                    $grandTotal_EWT = $unreports->sum('ewt');
                @endphp           
                <tfoot>
                    <tr class="font-bold">    
